...
`ifdef ViDBo
    input wire [36:0]  i_data,
    output wire [36:0] o_data,
    output wire        tf_push,
...
`elsif Pipeline
    ...
    output wire [36:0] o_data,
`else
    inout wire [36:0]  io_data,
`endif
    output wire[5:0] pwmLed,
    output wire[3:0] redVga,
    output wire[3:0] greenVga,
    output wire[3:0] blueVga,
    output wire hSyncVga,
    output wire vSyncVga,
    input wire 	clk,
    input wire 	       rstn,
...
`include "wb_intercon.vh"
   ...
   (
      .wb_clk_i(clk),
      .wb_rst_i(wb_rst),
      .wb_cyc_i(wb_m2s_pwmLed_cyc),
      .wb_adr_i({2'b0,wb_m2s_pwmLed_adr[5:2],2'b0}),
      .wb_dat_i(wb_m2s_pwmLed_dat),
      .wb_we_i(wb_m2s_pwmLed_we),
      .wb_stb_i(wb_m2s_pwmLed_stb),

      .wb_dat_o(wb_s2m_pwmLed_dat),
      .wb_ack_o(wb_s2m_pwmLed_ack),
      .wb_err_o(wb_s2m_pwmLed_err),
...
`else
   ...
   bidirec gpio32 (.oe(en_gpio[32]), .inp(o_gpio[32]), .outp(i_gpio[32]), .bidir(io_data[32]));
   bidirec gpio33 (.oe(en_gpio[33]), .inp(o_gpio[33]), .outp(i_gpio[33]), .bidir(io_data[33]));
   bidirec gpio34 (.oe(en_gpio[34]), .inp(o_gpio[34]), .outp(i_gpio[34]), .bidir(io_data[34]));
   bidirec gpio35 (.oe(en_gpio[35]), .inp(o_gpio[35]), .outp(i_gpio[35]), .bidir(io_data[35]));
   bidirec gpio36 (.oe(en_gpio[36]), .inp(o_gpio[36]), .outp(i_gpio[36]), .bidir(io_data[36]));
`endif
  ...
  `ifdef ViDBo
        .ext_pad_i     ({27'b0, i_data[36:32]}),
        .ext_pad_o     ({27'b0, o_data[36:32]}),
  `elsif Pipeline
        .ext_pad_i     ({27'b0, i_data[36:32]}),
        .ext_pad_o     ({27'b0, o_data[36:32]}),
  `else
        .ext_pad_i     ({27'b0, i_gpio[36:32]}),
        .ext_pad_o     ({27'b0, o_gpio[36:32]}),
  `endif

        .ext_padoe_o   ({27'b0, en_gpio[36:32]}));
